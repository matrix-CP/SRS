...
namespace SRS.Repositories
{
    public class StudentRepository : CommonRepository,IStudentRepository
    {
        ...
        public List<tblStudent> FetchAllStudents()
        {
            ...
            conn.Open();
            using (var command = new NpgsqlCommand("SELECT s.c_studentid, s.c_name, s.c_birthdate, s.c_gender, s.c_address, s.c_language, s.c_course, s.c_profile, s.c_document, s.c_mobile, c.c_coursename FROM t_student_registration s INNER JOIN t_course c ON s.c_course = c.c_courseid WHERE s.c_mobile IS NOT NULL AND s.c_address IS NOT NULL AND s.c_document iS NOT NULL;", conn))
            {
